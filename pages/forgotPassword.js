    ...
    async function handleForgotPassword(data) {
      ...
      try {
        if (data) {
          ...
          let response = await axios.post(`${Baseurl + "/" + Endpoint.FORGOT_PASSWORD}`, data)
        ...
        <div className="relative bg-white flex flex-col min-h-screen items-center py-5 px-3 max-w-6xl mx-auto md:flex-row-reverse md:justify-center md:gap-[10rem]">
          ...
          <div className=''>
            <div className='text-center py-2 mb-8'>
              <h1 className='text-2xl font-bold md:text-4xl'>Forgot Password!</h1>
              <h2 className='text-xs text-gray-500/80 md:text-sm'>Enter your email to recover password</h2>
            </div>

            <form className='space-y-5' onSubmit={handleSubmit(handleForgotPassword)}>
                {error?.fromLogin && (
